...
class PerformanceAnalyzer:
            ...
            for operand in getattr(instruction, "operands", []):
                if isinstance(operand, str) and operand.startswith("[") and operand.endswith("]"):
                    try:
                        ...
                        self.metrics.memory_access_counts[addr] = (
                            self.metrics.memory_access_counts.get(addr, 0) + 1
                        )
                    except ValueError:
                        pass
    ...
    def get_report(self) -> dict[str, Any]:
        ...
        if self.metrics.memory_access_counts:
            sorted_mem = sorted(
                self.metrics.memory_access_counts.items(), key=lambda x: x[1], reverse=True
            )
        ...
        return {
            ...
            "hot_spots": {"opcode": most_used_instr, "memory": hot_memory},
    ...
    def _generate_suggestions(self) -> list[str]:
        ...
        if self.metrics.instruction_counts.get("MULT", 0) > 10:
            suggestions.append(
                "High MULT usage detected. Consider shift-and-add or pre-calculated tables."
            )
        ...
        for addr, count in self.metrics.memory_access_counts.items():
            if count > self.metrics.total_cycles * 0.5:
                suggestions.append(
                    f"Memory address [{addr}] is a bottleneck (accessed in >50% of cycles)."
                )
