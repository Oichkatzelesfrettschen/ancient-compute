...
class JavaParser:
    ...
    def _parse_annotation_decl(self, modifiers: list[str]) -> AnnotationDecl:
        ...
        while (
            ...
            _member_modifiers = self._parse_modifiers()
    ...
    def _parse_class_member(self) -> object:
        ...
        if (
            self._match(TokenType.IDENTIFIER, TokenType.TYPE_NAME)
    ...
    def _parse_stmt(self) -> Stmt:
        if self._match(TokenType.LBRACE):
            ...
            if (
                self._peek(1).type in (TokenType.IDENTIFIER, TokenType.TYPE_NAME)
                or self._peek(1).type == TokenType.LBRACKET
    ...
    def _parse_try_stmt(self) -> TryStmt:
        ...
        while self._match(TokenType.CATCH):
            ...
            exc_var = self._expect(TokenType.IDENTIFIER, TokenType.TYPE_NAME).value
            self._expect(TokenType.RPAREN)
        ...
        while True:
            if self._match(TokenType.DOT):
                ...
                if self._match(TokenType.IDENTIFIER, TokenType.TYPE_NAME):
                    ...
                    self._advance()
                    type_ = ReferenceType(
                        self._expect(TokenType.TYPE_NAME, TokenType.IDENTIFIER).value
                    )
                    args = self._parse_arguments()
    ...
    def _parse_primary(self) -> Expr:
        if self._match(TokenType.NUMBER):
            ...
            self._advance()
            try:
                return Literal(float(val)) if "." in val else Literal(int(val))
            except (ValueError, TypeError):
                return Literal(val)
        elif self._match(TokenType.STRING_LIT):
            return Literal(self._expect(TokenType.STRING_LIT).value)
        elif self._match(TokenType.CHAR_LIT):
            return Literal(self._expect(TokenType.CHAR_LIT).value)
        elif self._match(TokenType.BOOLEAN):
            return Literal(self._expect(TokenType.BOOLEAN).value == "true")
    ...
    def _parse_new_expr(self) -> Expr:
        ...
        try:
            ...
            if self._match(TokenType.LBRACKET):
                ...
                args = self._parse_arguments()
                return NewExpression(type_, [], args)
        except Exception:
            self.pos = checkpoint
            self.current_token = self.tokens[self.pos]
            type_ = self._parse_type()
    ...
    def _parse_modifiers(self) -> list[str]:
        ...
        while self._match(
            ...
            mods.append(self.current_token.value)
            self._advance()
        return mods
    ...
    def _parse_parameters(self) -> list[Parameter]:
        ...
        while self.current_token.type != TokenType.RPAREN:
            ...
            pname = self._expect(TokenType.IDENTIFIER, TokenType.TYPE_NAME).value
    ...
    def _expect(self, *types: TokenType) -> Token:
        if self.current_token.type not in types:
            raise SyntaxError(
                f"Expected {types}, got {self.current_token.type} at line {self.current_token.line}"
            )
