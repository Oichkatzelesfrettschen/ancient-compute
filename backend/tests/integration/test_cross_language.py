...
""",
    ...
    async def test_factorial_cross_language(
        self, language: str, code: str, api_client: httpx.AsyncClient
    ):
        """
        ...
        response = await api_client.post(
            "/api/v1/execute", json={"code": code, "language": language, "timeout": 10}
        )
        ...
        assert (
            "120" in result["stdout"]
        ), f"{language} produced incorrect output: {result['stdout']}"
    ...
    async def test_ir_consistency(self, api_client: httpx.AsyncClient):
        ...
        for language, code in self.FACTORIAL_IMPLEMENTATIONS.items():
            response = await api_client.post(
                "/api/v1/compile",
                json={"code": code, "language": language, "options": {"emit_ir": True}},
            )
...
""",
    ...
    async def test_fibonacci_cross_language(
        self, language: str, code: str, api_client: httpx.AsyncClient
    ):
...
class TestCompilationPipeline:
    ...
    async def test_c_compilation_pipeline(self, api_client: httpx.AsyncClient):
        ...
        response = await api_client.post(
            "/api/v1/compile", json={"code": code, "language": "c", "options": {"emit_ir": True}}
        )
...
class TestErrorHandling:
    ...
    async def test_c_syntax_error(self, api_client: httpx.AsyncClient):
        ...
        response = await api_client.post("/api/v1/execute", json={"code": code, "language": "c"})
    ...
    async def test_python_syntax_error(self, api_client: httpx.AsyncClient):
        ...
        response = await api_client.post(
            "/api/v1/execute", json={"code": code, "language": "python"}
        )
    ...
    async def test_haskell_type_error(self, api_client: httpx.AsyncClient):
        ...
        response = await api_client.post(
            "/api/v1/execute", json={"code": code, "language": "haskell"}
        )
...
class TestPerformanceBenchmarks:
    ...
    async def test_compilation_speed(self, api_client: httpx.AsyncClient):
        ...
        response = await api_client.post("/api/v1/compile", json={"code": code, "language": "c"})
